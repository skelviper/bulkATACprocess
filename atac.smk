configfile: "bulkATACprocess/config.yaml"
sampleList=config["sample"]
genomeVersion = config["genome"]
OUT=config["output"]
IN=config["input"]
shiftExtendSize=config.get("shiftExtendSize", [125])
...
rule detectAdapter:
    ...
    conda:
        "envs/main_env.yaml",
    shell:"""
        python bulkATACprocess/detect_adapter.py {input.fq1} > {output.adapter1}
        python bulkATACprocess/detect_adapter.py {input.fq2} > {output.adapter2}
"""
...
rule tss:
    ...
    tss_plot='bulkATACprocess/tss.py'
